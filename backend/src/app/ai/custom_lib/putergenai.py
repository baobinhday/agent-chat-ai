# https://github.com/Nerve11/putergenai/
import asyncio
import json
import logging
import ssl
...
# Constants for ElevenLabs TTS options (available for external import)
ELEVENLABS_VOICE_IDS = [
    "deC6NEXcbavaVWbzjgzb",  # Default voice
    "n5UxjYFlD5aLGVRI2HXk",
    "foH7s9fX31wFFH2yqrFa",  
    "21m00Tcm4TlvDq8ikWAM",
    "558B1EcdabtcSdleer40"
]

# ElevenLabs models (https://elevenlabs.io/docs/overview/models)
# low latency - eleven_flash_v2_5
...
class PuterClient:
    ...
    async def ai_chat(
        ...
        def check_used_model(data: Dict[str, Any], requested_model: str, strict: bool) -> str:
            used_model = None
            if "metadata" in data and "providerUsed" in data["metadata"]:
                used_model = data["metadata"].get("providerUsed", "unknown")
    ...
    def _build_tts_payload(
        ...
        """
        Build payload for text to speech synthesis.
        
        Args:
            text: Text to convert to speech
            provider: TTS provider ("elevenlabs" or "openai")
            voice: Voice ID for ElevenLabs or voice name for OpenAI (optional)
            model: Model name for ElevenLabs or OpenAI (optional)
        ...
        match provider:
            case "elevenlabs":
                # Use provided voice or default to first in list
                elevenlabs_voice = voice or ELEVENLABS_VOICE_IDS[0]
                # Use provided model or default to first in list
                elevenlabs_model = model or ELEVENLABS_MODELS[0]
            ...
            case _:
                # OpenAI TTS provider
                openai_voice = voice or "alloy"
                openai_model = model or "gpt-4o-mini-tts"
                openai_instructions = instructions or "Keep the delivery clear and friendly."
