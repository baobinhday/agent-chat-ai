...
class RedisCacheSettings(BaseSettings):
    ...
    REDIS_CACHE_URL: str = config(
        "REDIS_CACHE_URL", default=f"redis://{REDIS_CACHE_HOST}:{REDIS_CACHE_PORT}"
    )
...
class RedisQueueSettings(BaseSettings):
    ...
    REDIS_QUEUE_URL: str = config(
        "REDIS_QUEUE_URL", default=f"redis://{REDIS_QUEUE_HOST}:{REDIS_QUEUE_PORT}"
    )
...
class CORSSettings(BaseSettings):
    ...
    CORS_ALLOW_ORIGINS: str = config("CORS_ALLOW_ORIGINS", default="*")
...
class OpenMemorySettings(BaseSettings):
    ...
    enabled: bool = config("OPENMEMORY_ENABLED", cast=bool, default=False)
