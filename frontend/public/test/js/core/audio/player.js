...
export class AudioPlayer {
    ...
    getAudioContext() {
        if (!this.audioContext) {
            ...
            log('Created AudioContext, sample rate: ' + this.SAMPLE_RATE + 'Hz', 'debug');
        }
        ...
        try {
            if (typeof window.ModuleInstance === 'undefined') {
                if (typeof Module !== 'undefined') {
                    ...
                    throw new Error('Opus library not loaded, both ModuleInstance and Module do not exist');
                }
            }
            ...
            this.opusDecoder = {
                ...
                init: function () {
                    ...
                    if (err < 0) {
                        this.destroy();
                        throw new Error(`Failed to initialize Opus decoder: ${err}`);
                    }
    ...
    enqueueAudioData(opusData) {
        if (opusData.length > 0) {
            ...
            log('Received empty audio data frame, possibly end signal', 'warning');
    ...
    // Clear all audio buffer and stop playback
    clearAllAudio() {
        log('AudioPlayer: Clear all audio', 'info');

        // Clear receive queue (use clear method to keep object reference)
        this.queue.clear();
